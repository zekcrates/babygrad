...
class Conv(Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, bias=True, device=None, dtype="float32"):
        ...
        # Initialize weight with correct 4D shape
        from . import init
        ...
        if bias:
            ...
            self.bias = Parameter(
                init.rand(out_channels, low=-bound, high=bound, dtype=dtype)
            )
        else:
            self.bias = None
    ...
    def forward(self, x: Tensor) -> Tensor:
        # Input: NCHW format
        # Transpose to NHWC for your ops.conv
        ...
        # Add bias if present
        if self.bias is not None:
            # Reshape bias from (C_out,) to (1, 1, 1, C_out)
